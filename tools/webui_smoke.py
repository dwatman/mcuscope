...
def main() -> int:
    ...
    for line in (
        ...
        "'● session' button: start one, watch it become '■ <name>', see the marker divider",
        "  in the terminal, then stop it and check `mcu lines --session <name>` returns",
        "  only that span; the button returns to '● session' because the daemon's own",
        "  automatic run took over again (hover it to see which)",
        "Settings > Sessions lists that run and the automatic one (tagged 'auto',",
        "  'recording'), each with its line count; 'export' downloads a .db",
        "  (open it with sqlite3 and check the lines are there), 'delete' confirms first and",
        "  then removes the run's lines while leaving everything outside it alone",
