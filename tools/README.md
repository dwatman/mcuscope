...
- `webui_smoke.py` - manual smoke harness for the web UI (SPEC 9.1).
  ...
  - Then serves `http://127.0.0.1:8558/ui/` and stays running so you can eyeball each panel in a browser.
  - Run it from the host venv so the `mcuscope` package imports; pass `--port` if 8558 is taken, or `--no-wait` to run just the auto-checks and exit.
