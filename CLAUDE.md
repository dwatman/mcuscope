...
Python 3.10+ is required, a uv-managed 3.13 virtualenv lives at `host/.venv`. uv venvs have no `pip` - use `uv pip install`.
...
```bash
cd host
uv venv --python 3.13               # first-time; a bare `uv venv` may pick a <3.10 python
