...
- [ ] Flip the install instructions, which currently say the package is not on PyPI yet. Both files:
    - [ ] `README.md`, in **Install**
    - [ ] `host/README.md`, in **Install** - this one is the PyPI long description, so the "not on PyPI yet" line would otherwise be served *on the PyPI page itself*.
