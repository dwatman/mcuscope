...
This document exists so the decisions below are not re-derived and the two traps in it are not walked into.
`docs/SPEC.md` section 10 remains the authority on scope; this note records why that entry reads the way it does.
...
That has been amended: the argument for storing was that decoded text would become searchable through `/lines`, `/wait` and `/assert`, and it would not.
The two ways to make it searchable are both bad:
...
The transitive weight is mild (python-can's own core deps are `wrapt`, `packaging`, `typing_extensions`), but still weight the base install should not carry for a feature most users will not touch.
...
The parts a naive implementation misses.
...
- **The web UI is the flagship CAN surface, not the CLI.**
  ...
  A CLI-only `--decode` leaves the UI showing raw hex beside a CLI showing signal names: either accept that gap explicitly for one release, or price the endpoint plus the JS.
...
- **One DBC path per port is correct here, despite multi-DBC being normal in vehicle work.**
  A port is one serial link to one MCU relaying one bus, so per-port already is per-bus; a list is a cheap TOML migration later if a bench ever needs it.
...
The in-scope deliverable is readable signal text at a terminal, which mostly serves a human; the version that serves a blind AI agent is signals as trendable numbers feeding numeric asserts, which is exactly the half held out of scope above.
