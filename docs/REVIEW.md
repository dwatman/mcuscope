...
1. **Registry leg** - executes every sweep under the sweep discipline below and files the verdict lists.
...
- Sweep scripts named below are kept on the owner's machine in `~/tt-data/mcuscope-tools/sweeps/class-<n>/`.
...
- Also sweep: `grep -n "regex.compile\|os\.\(unlink\|remove\|stat\|path.getsize\)\|httpx.AsyncClient(" host/mcuscope/server.py host/mcuscope/update_check.py`, ruling each hit inside an `async def` (`calls.py`).
...
- Also sweep: `grep -rn "uvicorn.Config\|create_server\|start_server" host/mcuscope`, ruling each listener by the probe that fronts it; the socket grep cannot see uvicorn's own bind.
...
- Also sweep: the exception types each non-CLI callee (`pidfile`, `config`, `dirs`, `protocol`) can raise, against `main()`'s arms.
  - A strict-decoding `open(..., encoding=)` read raises `ValueError` (R13-1).
...
- Also sweep: `grep -rnE "sys\.platform|os\.name|platform\.system|hasattr\((socket|signal|os|errno), |getattr\((os|errno|signal|socket), |except (ImportError|ModuleNotFoundError)" host/mcuscope`.
...
- Also sweep: enumerate from the built wheel and sdist, `git ls-files host/contrib firmware/monitor tools` and `[project.scripts]`.
  - Name the job that runs each item as a user does: serving, the target compiler, the unpacked sdist.
...
- Also sweep (R16-1 to R16-3):
  - `next(` over a filtered sequence: PEP 479 re-raises `StopIteration` as `RuntimeError`;
  - a whole-command `die` ahead of a per-item guard;
  - a paging loop judged on its last item.
...
- Also sweep: route returns, the startup banner and log, and CLI result lines, not only `/status`; a path read back is still the request until it is reported resolved.
...
- Also sweep: probe each library (pyserial, regex, httpx, websockets, json, urllib) for what it raises, and rule the tuple where the error leaves the module.
  - A sibling diff finds only the classes some sibling already names.
...
- Also sweep: `grep -rniE "mirror|mirrors|mirrored|duplicated like" host/mcuscope --include=*.py --include=*.js | grep -v vendor`, diffing each mirror's preconditions and input domain, not only its clauses.
...
- Also `GET /can/frames?port=` and `?last_ms=`, where the filter lands on the joined `lines` table.
  ...
  `CROSS JOIN` pins the drive order, and costs nothing on the filters that were already fast. `GET /plot/channels?port=` had the same shape; it now serves from the plot summary, so explain its rebuild (`_scan_plot_rows`, pinned by `test_store_plot_summary.py`), not `Store.query_plot_channels`, which no handler calls.
...
- Also sweep: drive each list-valued parameter with one and several values, time `rowid>?` reads with a filter matching nothing, and record each traced statement's call site and thread (`probe20.py`).
...
- Also sweep D to L (registry-15-28.md, class 22):
  - JS `Number(` and unary `+`; `fromhex` and `struct.unpack`;
  - click and FastAPI walks for library-side `int()`/`float()` coercion;
  - `strip`/`split`/`isspace` on wire text, env reads, the firmware parsers.
...
- Also sweep: add `|\.disabled\s*=\s*(on|none)\b` for holds written through a variable.
...
- Also sweep: on 3.10 `rollback()` resets every cursor on the connection (the next fetch raises `InterfaceError`), so a cursor held across a possible rollback fails on the floor only.
...
- Also sweep: `grep -nE 'canModel\(\)|of rows\)|chart\.names\.(forEach|map|filter)|soloShow\(' host/mcuscope/webui/*.js`; the entry's `\bcanModel\(\)\b` never matches.
...
- Also sweep the evictions themselves, `grep -nE 'splice\(0,|\.shift\(\)|\.delete\(|\.clear\(\)' host/mcuscope/webui/*.js`: a store's cap need not be named MAX or CAP.
...
- Also sweep:
  - a transport double answering every path and method is paired with an assertion on the request;
  - a double ignores no argument the real call consumes (`grep -nE "\(void\)[a-z_]+;" firmware/tests/fake_shims.c`).
...
- Also sweep: `grep -n "suppress(" host/tests/*.py`, and asserts in doubles, thread targets or handlers a broad except can swallow (`c28_nested.py`); a `match=` or a positive control (class 78).
...
- Also sweep: grep each refusal's text unique to it, not wording it shares with the next refusal on the path, then mutate the guard (R29-1 passed on the shared word "port").
...
- Also sweep: cross-module writes, `<module>.<name> =` and `setattr(<module>, ...)` outside monkeypatch (R32-1).
...
- Also sweep: in-process `cli.main([... "daemon", "start"|"restart" ...])`, whose spawn is in the package rather than the test.
...
- Also sweep: object-literal tables with content (`(const|let|var) X = {`) and `Object.fromEntries`.
...
- Also sweep: `grep -rnE "next_[a-z_]*\s*(\+=|=.*\+)|_due_beats\(|max\(next_|owed" host/mcuscope`: schedules advance in straight-line helpers, not `while` conditions.
...
- Sweep: `uv venv --python <floor> <scratch>/low && VIRTUAL_ENV=<scratch>/low uv pip install --resolution lowest-direct -e '.[dev]'`, then the full suite and `<scratch>/low/bin/ruff check .` there; each floor either passes or moves up. Also grep every `except ImportError` for a bare `return`/`pass`: an optional import that gates a documented property logs when it is missing.
...
- Also sweep: the save function behind each PUT, every key the file can hold under the collection, not only the model's keys.
...
- Also sweep: `grep -n '_field(' host/mcuscope/cli*.py` without `optional=True`, which exits 1 on an absent young field.
...
- Also sweep: `grep -n 'mon_buf_init(&b, resp, resp_max)' firmware/monitor/*.c`, bounding each payload by `MON_OK_PAYLOAD_MAX` (R48-1).
...
- Also sweep: every wrapped real in tests (`real_\w* = |orig\w* = |original = |_real = `), for spins on something other than an Event and `Event.wait()` parkers.
...
- Also sweep: `grep -n 'user_dir(' host/mcuscope/*.py` in place of `user_data_dir`, plus the names built from `pid_file_path`.
...
- Also sweep: f-string and IfExp query builders, and behaviours a CLI contract relies on (R53-3), not only parameters.
...
- Also: "two builders" means parameters with a client-side shape (list encoding, joined ids, time format) or a filter set rebuilt in two places, not every scalar passthrough.
...
- Also sweep: membership tests (`in ("enum", "bit")`) and kind-keyed dispatch tables (`LANE_KINDS[...]`); ASGI `scope["type"]` and DOM `.type` are noise.
...
- Tighter sweep: `grep -nE "\.(moved|base)\b|_last\b|prevTick|share_changes|_changed\(|changedBytes" host/mcuscope/webui/*.js host/mcuscope/cli*.py host/mcuscope/server.py`.
...
- Also (`c58b.py`):
  - allow digits in command names (`mcu i2c`), and find typer groups by `hasattr(cmd, "commands")`;
  - take `section.key` only in backticked or `[section]` context, and check `mcu <cmd> --flag` mentions.
...
- Also: diff against the round base as a multiset over `httpx\.Response\(|run_mcu_canned\(|recorder\(monkeypatch` and the JS canned bodies, bodies held in constants included.
...
- Tighter sweep: `grep -n "subscribe(\|stop_subscribers\|subscribers_closed\|None in rows" host/mcuscope/*.py`.
...
- Also sweep: `SetConsoleCtrlHandler` callbacks, which run on their own thread.
...
- Also sweep: `_pin_ceiling` and `id_ceiling`, freezes not spelled `max_id()`.
...
- Also sweep: `open(self.path` and `_OutFile(`, the lazy open every text export writes through.
...
- Also sweep: `status in (...)` and range checks; a WS close code sent before accept never reaches a client as that code.
...
- Also sweep: `grep -nE "\.value = |\.checked = " host/mcuscope/webui/*.js` over every dialog, ruling each control a later save reads back (`.trim()`, defaults and normalisations transform too).
...
- Also sweep: `grep -n "\.focus(\|showModal\|\.select()\|autofocus\|window.prompt" host/mcuscope/webui/*.js host/mcuscope/webui/index.html`.
...
- Also sweep: `grep -n "\.then(\|new Promise" host/mcuscope/webui/*.js`: a `.then(` chain is async without the keyword.
...
- Also sweep: `grep -nwiE "cap|limit|max_[a-z_]+|MAX_[A-Z_]+" host/mcuscope/webui/*.js host/mcuscope/cli*.py | grep -iE "textContent|title|print|err\(|die\(|echo"`.
...
- Also sweep: dict constants and names with digits (`^[A-Z_0-9]+ = [\[\(\{]`, `^const [A-Z_0-9]+ = `), and inline `parametrize` lists of flags and paths.
...
- Also: a site is ruled mechanically when its test asserts the same expression or root variable positively (`c78py.py`, `c78js.mjs`); only the residue is read.
...
- Also sweep: add `buffer|history|seed` to the pattern (the shared ring, history pages, seeds).
