...
Legs (evidence in `docs/review/2026-09-12-adversarial/`, all read-only against throwaway stacks): daemon (`daemon.md`, 1.05M-line capture, 31 mutations), CLI (`cli.md`, 121 commands driven, 12 mutations), web UI (`webui.md`, 29 mutations), host improvement sweep (`improve-host.md`, py-spy profile, import time, query plans), web UI improvement sweep (`improve-webui.md`, payload and render measurements). Fix-diff leg over the round's whole diff (`fix-diff.md`).
Yield, adversarial: 2 HIGH, 11 MED, 16 LOW, 5 nit, after ruling out the candidates each file lists.
- HIGH: `/can/frames?id=A,B` built `can_id IN (...)`, which drove the planner off the line-id order and through a temp b-tree per page (266 ms against 0.26 ms at 300k frames, 47 s against 3.9 s for the CSV at 1M lines; D1, class 20); the terminal pane export comma-joined `chan` where the endpoint takes it repeated, so two to five ticked channels were a 422 and nothing downloaded (W1).
- MED, behaviour: an open session's bundle had `capture.db` spanning more rows than every other member (D2); the export filename anchored `last_ms` at now, naming a window backwards (D3, class 17); `cmdModes` on a plain object, so an alias named `constructor` posted `/cmd` to a raw port (W2, class 34); the command bar's `auto` did not follow the daemon's sole-connected-port rule the same diff added (W3); a surface paused before its first line exported `id_to=0`, a 422 (W4); `changes` without `decode` reachable from the dialog, a 400 (W5); the streamed `log export` counted newlines not rows (C1); `--deadband` took `inf`, `nan` and non-ASCII digits (C2, class 22); `--to` silently unbounded against a pre-0.4.0 daemon (C3, new class 53).
- MED, tests: the export dialog's fake fetch accepted every URL the real endpoints refuse (W6, class 27); `exportPane` untested at all (W7, class 29).
- LOW and nit: D4-D10, C4-C10, W8-W14, each in its file; six daemon mutation survivors (V3, V10, V18, V19, L1, S4: the entire sole-connected-port change in `serial_link.py` was unpinned), four web UI (W7, W12, W13), one CLI (`newline=` on the export open, green across all 1465 tests).
Repeat-class instances: 17 (D3), 20 (D1), 22 (C2), 27 (W6), 29 (C6, D9, W7, W13, the survivors), 31 (D5), 34 (W2), 45 (attach dialog, P5), 46 (C3's mirror).
Ruled out by driving: class 49 on `/lines/export` (a mid-stream failure reaches the client as an incomplete chunked read over a real socket; the leg's first reading was withdrawn); no new export path blocks the loop (max stall 43 ms during a 52 MB streamed export, baseline 3-5 ms); the in-memory export branch materialises on the loop but no daemon configuration uses `:memory:` (accepted, test-only).
Yield, improvement: 28 proposals taken of 28 filed (13 host, 15 web UI), one not built (P11, per-chart height drag: expand exists and the persistence path is not worth its validation). Ranked picks: httpx's own CLI was 55 `rich` modules and 45 ms off every `mcu` call; a typo in `config.toml` loaded silently onto the default port and db; freed SQLite pages were never reclaimed once the cap was met and never on the age sweep (a 129 MB file stuck at 121 MB with 19 MB live); a daemon stopping under `mcu wait` was `Internal Server Error` exit 1 instead of exit 3; a drag zoom applied to one chart and broke the shared x axis SPEC 9.2 promises; the CAN table computed the payload diff and discarded it; the port chip never showed `target` (bench feedback item 5, built daemon-side and never reaching the browser).
Fixes: five batches partitioned by file (sim; CLI; daemon; web UI charts and terminal; web UI panels and dialogs), agents editing only their own files with no commits, the orchestrator reconciling one cross-batch test (`test_port_health`: the version probe is one more request) and merging the CHANGELOG lines. Every fix with a test verified to fail on hand-revert: 17 CLI, 27 daemon, 23 web UI charts, 30 web UI panels, 3 sim, all caught. Interfaces set in the briefs: the daemon's 503 on shutdown to the CLI's exit 3; `filterPaneTo` from terminal.js to can.js; `id_to` floor to 0 daemon-side so the dialog's `id_to=0` stays.
Gates: full suite 1563 passed, 1 skipped (was 1378 at the last round), ruff clean, JS suite 406 passed (was 328), no em or en dash in the tree, no dependency added.
Fix-diff leg (`fix-diff.md`, over the uncommitted round diff): 1 HIGH, 2 MED, 1 LOW, 1 nit, all fixed in-session with tests. HIGH: the shutdown 503 never fired on a real stop, because `store.stop()` runs in the lifespan finaliser, which uvicorn reaches only after its graceful wait has cancelled the parked handler; the test had driven `store.stop()` on a serving app, an order production cannot produce (F1: the sentinel now goes out from a `uvicorn.Server.handle_exit` override, which both SIGTERM and `/shutdown` reach; the test goes through that hook, and a real SIGTERM under `mcu wait` was driven to exit 3). MED: the version gate covered `--from/--to` only, while `--decode/--changes/--deadband` and `can dump --csv` are dropped just as silently by a 0.3.0 daemon (F2, class 53's own sweep); after a cancelled token prompt every streaming export navigated and saved the 401 body under the export's name (F3, class 55's shape in JS). LOW: the deadband refusal keyed on the name's last-declared kind while `_render` labels only that stream's rows, a false 400 when another stream declares the name numeric (F4). Nit: an alias derived from a serial number outside the grammar was refused naming `alias` (F5). Twelve of the batches' revert-verifications re-done by the leg, all caught; cross-batch seams driven green except F1.
New registry classes: 53 (a bound sent as a query parameter the peer may not declare), 54 (a wire shape built by hand at two sibling sites), 55 (a refusal keyed on a kind name where the behaviour is keyed on a rendering property). Sweeps run this session by the fix-diff leg, verdict lists in `fix-diff.md`: 53 (67 sites, 5 violators, all F2, fixed), 54 (16 parameter groups over the same sites, 1 violator: `chan` scalar in the CLI against repeated in the UI, a Python/JS pair whose forms differ only in arity, ruled compliant after the registry wording was narrowed to two builders on one side), 55 (48 sites, 1 violator, F4, fixed).

### Decisions and not built
- C1 closed at the root: a stored row is one line, so the store's single write path replaces CR and LF inside `raw` with a space; the four `wrote N` counters keep counting newlines. A row-count response header was the alternative and was not built.
- W4 closed daemon-side: every `id_to` floor is 0 (SPEC 3.4 never floored it); a freeze taken before the first line is an empty window.
- D4: `raw` and `dir` are carried through the CSV unchanged; the formula guard stays on device-declared cells only.
- D6: the ceiling is `MAX(id) WHERE ts <= ?`; the floor keeps the monotonic-ts assumption, now stated in SPEC 3.4.
- The bundle holds the sweep lock from freezing `hi` to the end of the build, so a purge or size sweep waits for a bundle in progress; a multi-minute bundle stalls retention for that long (accepted).
- Improvement 12: the writer gets a 64 MB page cache; `wal_autocheckpoint=4000` (a further 27% at flood rate, a 16 MB WAL sidecar and a worse commit tail) is left for the owner.
- P7: the narrated `ERR`-shaped sim line is plain debug text; an `err`-styled line needs a real response frame, which "no new wire syntax" forbids.
- P6 against W9: with no token the four streaming exports navigate an `<a download>` (the browser streams to disk), so a daemon refusal cannot show inline there; the dialog's own guards, now pinned by the hardened double, keep a refused URL from being built; the bundle and the token case keep fetch-to-blob with inline errors.
- `fillSessions` is awaited by Export, not before the dialog opens.
- `-o -` is refused on every export command (the sibling commands wrote a file named `-`); stdout is the no-`-o` form.
- Not built: a `--doctor` subcommand, dropping `idx_lines_ts`, a leaner `/plot/series` shape, trimming `dir`/`seq` from the WS row, terminal find-and-jump, the writer off the loop, per-lane digital export, `POST /purge` in Settings (each with its one-line reason in the improvement files).

### The two questions (round close)
Q1, least confident: C10 (stdout CRLF on Windows) and the `.err` file claims are reasoned, not driven, and owed to the Windows leg with the whole suite; D6's backwards-clock condition was injected, not produced on a live daemon; the CLI leg's throwaway daemon exited once with no message and no crash log, most likely swept by a concurrent full-suite run, not chased. Second: the bundle-under-sweep-lock fix was driven by the batch with a concurrent purge, but not with a size sweep firing on its own tick.
Q2, the gap: the browser. Every web UI change this round has its DOM-free test and a manual-verify line (36 lines, `manual-verify.md`), and none has been run in a browser: the shared zoom under uPlot cursor sync, the CAN freeze, the `<a download>` navigation, the attach dialog fields, the port chip rate. The other direction of the export contract (what the daemon's responses do to the browser: `Content-Disposition` parsing was checked and is fine, a multi-MB blob is now avoided by P6) is the one no leg drove end to end.

### Carried open
- The browser checklist: the 2026-09-07 items plus this round's 36 lines in `docs/review/2026-09-12-adversarial/manual-verify.md`.
- Windows: the full suite, C10 (`mcu log export --csv > f` against `-o f` sizes), the `.err` file claims, class 8/13.
...
**Resolved 2026-08-28 (owner ruling: fix)**: `POST /ports` accepted `socket://`/`rfc2217://` devices without the config-write bar, so on a tokenless non-loopback bind a network client could point a port at an attacker host (tx exfiltration + forged rx into the capture). Now held to `_config_write_denied`, SPEC 3.4 updated; `DELETE`/reconnect stay open since neither can name a new destination. Existing loopback test clients pass the bar unchanged; one non-loopback refusal test added.
