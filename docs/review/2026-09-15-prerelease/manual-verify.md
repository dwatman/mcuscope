...
- [ ] E-6: `kill -STOP` the daemon; the gear opens Settings read-only about 2 s later, `+ Attach` opens with "no reply from daemon"; `kill -CONT` recovers.
...
- [ ] FW-1: `kill -STOP` the daemon; pane export, Export, Cancel, open a chart export. Its Export is enabled; about 2 s later the list shows `whole capture` and `could not list sessions: no reply from daemon`. After `kill -CONT` nothing downloads from the cancelled dialog.
...
- [ ] Export preflight: daemon `kill -STOP`ped, Export shows `no reply from daemon` after about 2 s; after `kill -CONT` nothing downloads.
...
- [ ] Dialogs: `kill -STOP` the daemon, click the gear, then click into the command input: focus stays there; Settings reads "loading..." until 2 s, then read-only. Same for `+ Attach` with "loading devices...".
...
- [ ] Clear-all during a backfill: needs a daemon slow on `/lines` with the page already loaded (a large capture over a slow link); a stalled daemon cannot serve the page. Not runnable with the sim.
- [x] FD2-1: fields held while loading. Passed 2026-09-15.
- [x] FD2-2: failed 2026-09-15 (the caret jumped to the token box below the notice); fixed: no focus move, the notice is a fixed banner. Recheck: `kill -STOP`, click the gear, wait past 2 s: an orange banner at the top of the dialog, caret unmoved.
