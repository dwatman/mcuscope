...
- [ ] [CI] `test_a_lost_race_stop_reaches_the_interpreter_behind_the_venv_launcher` passes on `windows-latest`: terminating the venv launcher ends the interpreter. A skip fails the job (`ci.yml`).
...
- [ ] [CI] `test_pidfile.py` on `windows-latest`: `test_neither_path_replaces_a_newer_record[True]`, `test_create_record_never_replaces` and the straddle tests pass, not skipped. It is the only run of the `os.rename` no-replace put-back.
