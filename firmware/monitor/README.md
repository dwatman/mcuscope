...
The portable, RTOS-free MCU monitor module (SPEC section 5).
Handles the UART line protocol (parse, dispatch, responses, `!can`/`!p`/plot events) and calls out to a small set of bus shims the integrating project implements.
C99, no dynamic allocation, no HAL/LL/CMSIS in the core, static buffers only.
